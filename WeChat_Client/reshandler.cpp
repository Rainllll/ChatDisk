...
void ResHandler::initSetting(QString caname)
{
    QString path = QString("%1/%2").arg(Client::getInstance().getRootPath()).arg(QString(caname));
    QDir dir;

    if(dir.mkdir(path))
    {
        qDebug() << "用户文件夹创建成功";
        QString pathMySetting = QString("%1/MySetting").arg(path);

        if(dir.mkdir(pathMySetting))
        {
            qDebug() << "用户设置文件夹创建成功";
        }
        else
        {
            qDebug() << "用户设置文件夹创建失败或已存在";
        }

        QString settingMyPic = QString("%1/MyPic").arg(pathMySetting);

        if(dir.mkdir(settingMyPic))
        {
            qDebug() << "用户设置头像文件夹创建成功";
        }
        else
        {
            qDebug() << "用户设置头像文件夹创建失败或已存在";
        }
    }
    else
    {
        qDebug() << "用户文件夹创建失败或已存在";
    }
}
...
void ResHandler::InitUserDir()
{
    QString sysfilePath = QString("%1/sysfile/%2").arg(Client::getInstance().getRootPath()).arg(Client::getInstance().m_stringLoginName);
    QDir dir(sysfilePath);

    if (!dir.exists())
    {
        if (dir.mkpath(sysfilePath))
        {
            qDebug() << "sysfile目录初始化成功";
        }
        else
        {
            qDebug() << "sysfile目录初始化失败";
        }
    }
    else
    {
...
void ResHandler::initFriend(QString caname)
{
    //创建好友文件夹
    ...
    //初始化好友Pic
    QString friendPic = QString("%1/UserPic").arg(path);
    //发送给服务器初始化成功信号 准备接收头像
    //服务器转发信号
    //接收信号开始传输
    //传输成功
}

void ResHandler::initPic(QString caname)
{
    //初始化好友Pic
